...
namespace Stitcher360
{
	class PictureAccesser
	{
		...
		{
			// The location of the ray intersecting the picture, which translates to the intersection of a line and a plane in analytics
		...
		{
			// Generate a rotation matrix from two angles this V minus is needed to flip the coordinate system
			...
			output[0] = (int)Math.Round((absPositionInNewBasis[1] * sessionData.NewBasisScale));
			output[1] = (int)Math.Round((absPositionInNewBasis[2] * sessionData.NewBasisScale));
			return output;
		}
	}
}
